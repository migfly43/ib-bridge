...
var Incoming = /** @class */ (function () {
        ...
        if (version >= 5) {
            ...
            if (secIdListCount > 0) {
                ...
                while (secIdListCount--) {
                    ...
                    contract.secIdList.push(tagValue_1);
                }
            }
        }
    ...
    Incoming.prototype._CONTRACT_DATA = function () {
        ...
        if (version >= 7) {
            secIdListCount = this.dequeueInt();
            console.log("sdfasdfasdf" + secIdListCount);
            if (secIdListCount > 0) {
                contract.secIdList = [];
        ...
        if (version >= 29) {
            ...
            if (comboLegsCount > 0) {
                ...
                for (i = 0; i < comboLegsCount; ++i) {
                    var comboLeg_1 = {};
                    comboLeg_1.conId = this.dequeueInt();
                    comboLeg_1.ratio = this.dequeueInt();
                    comboLeg_1.action = this.dequeue();
                    comboLeg_1.exchange = this.dequeue();
                    comboLeg_1.openClose = this.dequeueInt();
                    comboLeg_1.shortSaleSlot = this.dequeueInt();
                    comboLeg_1.designatedLocation = this.dequeue();
                    comboLeg_1.exemptCode = this.dequeueInt();
                    contract.comboLegs.push(comboLeg_1);
                }
            }
        ...
        if (version >= 26) {
            ...
            if (smartComboRoutingParamsCount > 0) {
                ...
                for (i = 0; i < smartComboRoutingParamsCount; ++i) {
                    ...
                    order.smartComboRoutingParams.push(tagValue_2);
                }
            }
        }
        if (version >= 15) {
            ...
            if (!_.isEmpty(order.algoStrategy)) {
                ...
                if (algoParamsCount > 0) {
                    ...
                    for (i = 0; i < algoParamsCount; ++i) {
                        var tagValue_3 = {};
                        tagValue_3.tag = this.dequeue();
                        tagValue_3.value = this.dequeue();
                        order.algoParams.push(tagValue_3);
                    }
                }
            }
        }
    ...
    Incoming.prototype._ORDER_STATUS = function () {
        ...
        this._emit("orderStatus", id, status, filled, remaining, avgFillPrice, permId, parentId, lastFillPrice, clientId, whyHeld);
    };
    Incoming.prototype._PORTFOLIO_VALUE = function () {
        ...
        this._emit("updatePortfolio", contract, position, marketPrice, marketValue, averageCost, unrealizedPNL, realizedPNL, accountName);
    };
    ...
    Incoming.prototype._TICK_EFP = function () {
        ...
        this._emit("tickEFP", tickerId, tickType, basisPoints, formattedBasisPoints, impliedFuturesPrice, holdDays, futureExpiry, dividendImpact, dividendsToExpiry);
    };
