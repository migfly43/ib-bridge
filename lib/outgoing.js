...
var Outgoing = /** @class */ (function () {
    ...
    ;
    Outgoing.prototype.exerciseOptions = function (tickerId, contract, exerciseAction, exerciseQuantity, account, override) {
        var version = 2;
            ...
            if (_.isArray(contract.comboLegs)) {
                contract.comboLegs.forEach(function (comboLeg) {
                    if (comboLeg.shortSaleSlot !== 0 || !_.isEmpty(comboLeg.designatedLocation)) {
                        return _this._controller.emitError('It does not support SSHORT flag for combo legs.');
                    }
                });
            }
        }
            ...
            if (_.isArray(contract.comboLegs)) {
                contract.comboLegs.forEach(function (comboLeg) {
                    if (comboLeg.exemptCode !== -1) {
                        return _this._controller.emitError('It does not support exemptCode parameter.');
                    }
                });
            }
        }
            ...
            if (_.isArray(order.orderComboLegs)) {
                order.orderComboLegs.forEach(function (orderComboLeg) {
                    if (orderComboLeg.price !== Number.MAX_VALUE) {
                        return _this._controller.emitError('It does not support per-leg prices for order combo legs.');
                    }
                });
            }
        }
        ...
        if (this._controller._serverVersion >= 8 &&
            ...
            if (!_.isArray(contract.comboLegs)) {
                ...
                contract.comboLegs.forEach(function (comboLeg) {
                    ...
                    if (_this._controller._serverVersion >= C.MIN_SERVER_VER.SSHORT_COMBO_LEGS) {
                        args.push(comboLeg.shortSaleSlot);
                        args.push(comboLeg.designatedLocation);
                    }
                    if (_this._controller._serverVersion >= C.MIN_SERVER_VER.SSHORTX_OLD) {
    ...
    ;
    Outgoing.prototype.reqHistoricalData = function (tickerId, contract, endDateTime, durationStr, barSizeSetting, whatToShow, useRTH, formatDate, keepUpToDate) {
        var version = 6;
    ...
    ;
    Outgoing.prototype.debugMsg = function (msg) {
