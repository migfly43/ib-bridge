...
var ib = new (require('..'))({
    ...
    console.log('%s %s%s', chalk.cyan('[accountUpdateMultiEnd]'), chalk.bold('reqId='), reqId);
}).on('accountUpdateMulti', function (reqId, account, modelCode, key, value, currency) {
    console.log('%s %s%s %s%s %s%s %s%s %s%s %s%s', chalk.cyan('[accountUpdateMulti]'), chalk.bold('reqId='), reqId, chalk.bold('account='), account, chalk.bold('modelCode='), modelCode, chalk.bold('key='), key, chalk.bold('value='), value, chalk.bold('currency='), currency);
});
ib.connect();
var requestId = 0;
