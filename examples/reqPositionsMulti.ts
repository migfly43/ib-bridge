...
var ib = new (require('..'))({
    ...
    console.log('%s %s%s %s%s %s%s %s%s %s%s %s%s', chalk.cyan('[positionMulti]'), chalk.bold('reqId='), reqId, chalk.bold('account='), account, chalk.bold('modelCode='), modelCode, chalk.bold('contract='), JSON.stringify(contract), chalk.bold('pos='), pos, chalk.bold('avgCost='), avgCost);
}).on('positionMultiEnd', function (reqId) {
    console.log('%s %s%s', chalk.cyan('[positionMultiEnd]'), chalk.bold('reqId='), reqId);
});
ib.connect();
var requestId = 0;
