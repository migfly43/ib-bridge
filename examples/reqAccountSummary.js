...
// port: 7496
    ...
    console.log('%s %s%d %s%s %s%s %s%s %s%s', chalk.cyan('[accountSummary]'), chalk.bold('reqId='), reqId, chalk.bold('account='), account, chalk.bold('tag='), tag, chalk.bold('value='), value, chalk.bold('currency='), currency);
}).on('accountSummaryEnd', function (reqId) {
    console.log('%s %s%d', chalk.cyan('[accountSummaryEnd]'), chalk.bold('reqId='), reqId);
});
ib.connect();
