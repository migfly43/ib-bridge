...
// port: 7496
    ...
    console.log('%s %s%s %s%s', chalk.cyan('[contractDetails]'), chalk.bold('reqId='), reqId, chalk.bold('contract='), JSON.stringify(contract));
}).on('contractDetailsEnd', function (reqId) {
    console.log('%s %s%s', chalk.cyan('[contractDetailsEnd]'), chalk.bold('reqId='), reqId);
});
ib.connect();
// Forex
